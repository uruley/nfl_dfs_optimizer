...
class WRMLTrainer:
    ...
    def engineer_wr_features(self, wr_data, pbp):
        ...
        for player_id in wr_data['player_id'].unique():
            ...
            # WR efficiency metrics (FIXED)
            if 'receiving_yards' in player_data.columns and 'targets' in player_data.columns:
                yards_per_target = player_data['receiving_yards'] / player_data['targets'].replace(0, 1)
                player_data['yards_per_target_avg_8'] = yards_per_target.rolling(8, min_periods=1).mean().shift(1)
            else:
                player_data['yards_per_target_avg_8'] = 8.5

            if 'receptions' in player_data.columns and 'targets' in player_data.columns:
                catch_rate = player_data['receptions'] / player_data['targets'].replace(0, 1)
                player_data['catch_rate_avg_5'] = catch_rate.rolling(5, min_periods=1).mean().shift(1)
            else:
                player_data['catch_rate_avg_5'] = 0.65

            # Fix target share and air yards share
